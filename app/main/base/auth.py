# -*- coding:utf-8 -*-
from flask import jsonify, current_app, request, url_for, redirect, g, make_response
...
def parse_token(username_or_token):
    data = {}
    try:
        data = serializer.loads(username_or_token)
        code = 1
    except SignatureExpired:  # 认证超时
        code = 2
    except BadSignature:  # 解析错误超时
        code = 3
    return data, code
                ...
                if resp and isinstance(resp, dict) and "success" in resp and "uid" in resp:
                    if resp["success"] is True:
                        ...
                        if userinfo.get('identifier'):
                            ...
                            # user = User.get(username=userinfo['identifier'])
                            if user is None:
